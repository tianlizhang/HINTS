...
def main(args):
    ...
    trainloader, _, testloader = data_loader(args, coauthors, features, 0.75, 0.25)
    ...
    for epoch in range(100):
        ...
        for history_gs, history_xs, graph, idx in trainloader:
            # history_gs = coauthors[:idx]
            # history_xs = features[:idx]
            # graph = coauthors[idx]
            # logger.info('Graph %s.', str(graph))
            logger.info('history_gs %s.', len(history_gs))
            print(len(history_gs))
            print(len(history_xs), history_xs[0].shape)
            print(idx, idx.shape)
            ...
            opt.step()
            # print(loss.item())
            loss_avg += loss.item()*len(history_xs)
            total += len(history_xs)
        loss_avg /= total

        y_probs = []
        y_labels = []
        # for idx in range(train_idx, len(coauthors)):
        for history_gs, history_xs, graph, idx in testloader:
            # history_gs = coauthors[:idx]
            # history_xs = features[:idx]
            # graph = coauthors[idx]
            negative_graph = construct_negative_graph(graph, k)
