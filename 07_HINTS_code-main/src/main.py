...
def main(beta,num):
    ...
    model = Model(adj_list, feature_list, embedding_size, input_seq, output_seq,
                  index_table_P1P, index_table_P1A, index_table_P1V, index_table_P1K,\
                      alignment_list,args.lr,args.batch_size,beta,\
                          args.gnn, args.method)
    model.build()
    ...
    if args.dataset == "v13":
       adj_list_, feature_list_, labels, index_table_P1P_, index_table_P1A_, index_table_P1V_,\
       index_table_P1K_,alignment_list_,label_max, label_min = load_data_dblpV13(2005,'train')
    ...
    with tf.Session() as sess:
        ...
        for epoch in outer:
            ...
            for idx, list in enumerate(minibatches):
                ...
                train_loss = train(sess, model, feed)
                outer.set_postfix(loss=train_loss)
        # print("training done!!")
        ...
        if args.dataset == "v13":
            adj_list_, feature_list_, labels, index_table_P1P_, index_table_P1A_, index_table_P1V_,\
            index_table_P1K_,alignment_list_,label_max, label_min = load_data_dblpV13(2010,'train')
        ...
        if args.gnn == 'gcn':
            np.savetxt("../experiment/result_gcn/"+args.dataset+"_test_test_beta_"+str(beta)+"num_"+str(num)+".txt", pred_test, fmt="%.2f %.2f %.2f %.2f %.2f")
            np.savetxt("../experiment/result_gcn/"+args.dataset+"_test_labels_"+str(beta)+"num_"+str(num)+".txt", labels, fmt="%.2f %.2f %.2f %.2f %.2f")
        elif args.method == 'seq2seq':
            np.savetxt("../experiment/result_seq2seq/"+args.dataset+"_test_test_beta_"+str(beta)+"num_"+str(num)+".txt", pred_test, fmt="%.2f %.2f %.2f %.2f %.2f")
            np.savetxt("../experiment/result_seq2seq/"+args.dataset+"_test_labels_"+str(beta)+"num_"+str(num)+".txt", labels, fmt="%.2f %.2f %.2f %.2f %.2f")
        else:
        ...
        # np.savetxt("../fanxing/result_seq2seq/"+args.dataset+"_test_test_beta_"+str(beta)+"num_"+str(num)+".txt", pred_test, fmt="%.2f %.2f %.2f %.2f %.2f")
        # np.savetxt("../fanxing/result_seq2seq/"+args.dataset+"_test_labels_"+str(beta)+"num_"+str(num)+".txt", labels, fmt="%.2f %.2f %.2f %.2f %.2f")
