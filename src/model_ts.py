...
class CVAE():
    ...
    def build_graph(self):
        ...
        with tf.variable_scope("decoder",reuse=False):
            #conditional
            _, self.final_state = tf.nn.dynamic_rnn(cell, self.imputed_embeds, initial_state=initial_state) # [p, 5, d] -> [p, 50]
            h1_imputed_emb_mean = tf.layers.dense(self.final_state, mlp_hidden_unit["conditional_1"], # [p, 50] -> [p, 20]
                                                  activation=tf.nn.relu, name="condition_mean_1")
            self.condition_mean = tf.layers.dense(h1_imputed_emb_mean, mlp_hidden_unit["encoder_2"], # [p, 20] -> [p, 10]
                                                  name="condition_mean_2")
            self.z = self.condition_mean
            ##decoder
            h1_decoder_eta = tf.layers.dense(self.z,mlp_hidden_unit["decoder_1"],activation=tf.nn.relu,name="eta_decoder1") #[p, 20]
            h2_decoder_eta = tf.layers.dense(h1_decoder_eta, mlp_hidden_unit["decoder_2"], activation=tf.nn.relu, name="eta_decoder2")#[p, 8]
            self.eta = tf.layers.dense(h2_decoder_eta,mlp_hidden_unit["decoder_3"],name="eta_decoder3") #[p, 1]
            ...
            self.citation_pred = tf.transpose(tf.reshape(self.citation_pred_list,[5,self.batch_size])) # [p, 5]
