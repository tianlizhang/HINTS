...
def load_data_aminer(year,flag):
    ...
    with open('../aminer/cumulative_log_labels_new.pkl','rb') as f: # len=13, 2005-2017的引用情况
    ...
    with open('../aminer/individual_data/index_' + str(year) + '.pkl', 'rb') as f:
        index = pkl.load(f) # list, len=4, [array(5, 105600, 100), (5, 105600, 20), (5, 105600, 1), (5, 105600, 15)]
        index_table_P1P = index[0][:,rank,:] # (5, 3000, 100)
        index_table_P1A = index[1][:,rank,:] # (5, 3000, 20)
        index_table_P1V = index[2][:,rank,:] # (5, 3000, 1)
        index_table_P1K = index[3][:,rank,:] # (5, 3000, 15)


    with open('../aminer/individual_data/alignment_nodes.pkl','rb')as f:
        alignment_ids = pkl.load(f) # list, len=10, 2000-2009
    alignment_index = [alignment_ids['aligment_id_'+str(year-(5-i))] for i in range(4)]
    # list[list[list]]: [ [[n], [n]], ... ] 每两年之间，不同关系节点的id对应关系
...
def load_data_dblpV13(year,flag):
    data_path = '../fanxing/'
    with open('../fanxing/individual_data/graph_' + str(year - 5) + '_nf.pkl', 'rb') as f:
        graph_1 = pkl.load(f)
        adj_ = [normalize(adj_)for adj_ in graph_1['adj']] # adj: list(sp.csr_matrix), 5个类型
        adj_1 = [convert_sparse_matrix_to_sparse_tensor(adj) for adj in adj_]
        feature_1 = graph_1['feature'] # np.array, [n, 4]

    with open('../fanxing/individual_data/graph_' + str(year - 4) + '_nf.pkl', 'rb') as f:
        graph_2 = pkl.load(f)
        adj_ = [normalize(adj_)for adj_ in graph_2['adj']]
        adj_2 = [convert_sparse_matrix_to_sparse_tensor(adj) for adj in adj_]
        feature_2 = graph_2['feature']
    
    with open('../fanxing/individual_data/graph_' + str(year - 3) + '_nf.pkl', 'rb') as f:
        graph_3 = pkl.load(f)
        adj_ = [normalize(adj_)for adj_ in graph_3['adj']]
        adj_3 = [convert_sparse_matrix_to_sparse_tensor(adj) for adj in adj_]
        feature_3 = graph_3['feature']

    with open('../fanxing/individual_data/graph_' + str(year - 2) + '_nf.pkl', 'rb') as f:
        graph_4 = pkl.load(f)
        adj_ = [normalize(adj_)for adj_ in graph_4['adj']]
        adj_4 = [convert_sparse_matrix_to_sparse_tensor(adj) for adj in adj_]
        feature_4 = graph_4['feature']
 
    with open('../fanxing/individual_data/graph_' + str(year - 1) + '_nf.pkl', 'rb') as f:
        graph_5 = pkl.load(f)
        adj_ = [normalize(adj_)for adj_ in graph_5['adj']]
        adj_5 = [convert_sparse_matrix_to_sparse_tensor(adj) for adj in adj_]
        feature_5 = graph_5['feature']
    ...
    with open('../fanxing/data/cumulative_log_labels_new.pkl','rb') as f: # len=13, 2005-2017的引用情况
        # 13个csv，每一列是year-2018的引用量
        labels = pkl.load(f)['P' + str(year) + '_label'].iloc[rank, 1:6].values # shape=(3000, 5)

    labels, label_max, label_min = lable_normorlization(labels)
    with open('../fanxing/individual_data/index_' + str(year) + '.pkl', 'rb') as f:
        index = pkl.load(f) # list, len=4, [array(5, 105600, 100), (5, 105600, 20), (5, 105600, 1), (5, 105600, 15)]
        index_table_P1P = index[0][:,rank,:] # (5, 3000, 100)
        index_table_P1A = index[1][:,rank,:] # (5, 3000, 20)
        index_table_P1V = index[2][:,rank,:] # (5, 3000, 1)
    ...
    with open('../fanxing/individual_data/alignment_nodes.pkl','rb')as f:
        alignment_ids = pkl.load(f) # list, len=10, 2000-2009
    alignment_index = [alignment_ids['aligment_id_'+str(year-(5-i))] for i in range(4)]
    # list[list[list]]: [ [[n], [n]], ... ] 每两年之间，不同关系节点的id对应关系
